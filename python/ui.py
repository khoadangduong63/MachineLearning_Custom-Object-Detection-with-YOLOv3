...
net = load_net(b"../cfg/custom.cfg", b"../custom_110000.weights", 0)
meta = load_meta(b"../cfg/custom.data")
...
class New_Toplevel:
    def __init__(self, gbTop=None):
        ...
        def clickRunEvent():
            ...
            color_list = {"0_finger":(255,0,0),"5_fingers":(0,0,255)}
            color = 0
            if predict == []:
                result.append("None object detected")
                ...
                for i in range(len(predict)):
                    ...
                    cv2.rectangle(self._imageOverlay, (x-w/2,y-h/2), (x+w/2,y+h/2), color_list[predict[i][0]], 3)
                    font = cv2.FONT_HERSHEY_SIMPLEX
                    cv2.putText(self._imageOverlay,predict[i][0], (x-w/2,y-h/2), font, 1, color_list[predict[i][0]], 2, cv2.LINE_AA)
                    self.opacity = 1.0
        ...
        def clickAboutEvent():
            infoForm = Tk()
            infoForm.geometry("400x200+500+200")
            infoForm.title("Information")
            infoForm.resizable(width=False, height=False)
            info = "Full name: Duong Dang Khoa\n\nID: 1512251\n\nFinal Project: Custom Object Detection with YOLOv3\n\nObjects: Detect 0_finger and 5_fingers\n\nLecturer: Ngo Minh Nhut"
            lb = Label(infoForm, text = info, font=("Helvetica", 12), justify=LEFT)
            lb.pack()
            infoForm.mainloop()
